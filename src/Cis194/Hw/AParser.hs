...
instance Applicative Parser where
  ...
  (Parser f) <*> (Parser b) = Parser (\s -> (f s) >>= applyPair)
    where applyPair (g,s) = fmap (first g) (b s)
...
intPair = pure (\x s y -> [x,y]) <*> posInt <*> (char ' ') <*> posInt
...
intOrUppercase :: Parser ()
intOrUppercase = (ignoreOutput posInt) <|> (ignoreOutput (satisfy isUpper))
  where ignoreOutput = ((pure (\x->())) <*>)

data ParsedValue = ParsedInt Integer 
                 | ParsedChar Char
                 deriving (Eq,Show)

intOrUppercase2 :: Parser ParsedValue
intOrUppercase2 = ((pure ParsedInt) <*> posInt) <|> ((pure ParsedChar) <*> (satisfy isUpper))
